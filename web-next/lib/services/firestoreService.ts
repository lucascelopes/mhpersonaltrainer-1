...
import { isPremiumUserRecord } from './aiAccess';
...
export interface AdminUserSummary {
  ...
  name: string;
  email: string;
  role: 'admin' | 'personal' | 'academy' | 'aluno';
  createdAt?: Date;
}
...
const isValidPersonalCode = (value?: string | number | null) => {
...
async function getCodigoPersonal(uid: string): Promise<string | null> {
  try {
    ...
    if (!personalSnapshot.empty) {
      const code = personalSnapshot.docs[0].data().codigoPersonal;
    ...
    if (!professorSnapshot.empty) {
      const code = professorSnapshot.docs[0].data().codigoPersonal;
      if (isValidPersonalCode(code)) {
        return String(code);
      }
    }
    ...
    if (userSnap.exists()) {
      ...
      if (isValidPersonalCode(data.codigoPersonal)) {
        return String(data.codigoPersonal);
      }
    }
  ...
  try {
    ...
    const addAluno = (docSnap: any) => {
      ...
      alunosMap.set(docSnap.id, {
        ...
        vinculadoPorAcademia: toBool(data.vinculadoPorAcademia),
        personalVinculadoEm: data.personalVinculadoEm?.toDate(),
    });
  };
...
async function getAvaliacoesDoAluno(uid: string): Promise<Avaliacao[]> {
  try {
    ...
    const getText = (value: unknown) => (typeof value === 'string' ? value : undefined);
    return result.data.slice(0, 5).map((evaluation) => ({
      id: evaluation.id,
      tipo: evaluation.type,
      data: evaluation.date,
      peso: 'peso' in evaluation ? getNumber((evaluation as { peso?: unknown }).peso) : undefined,
      altura: 'altura' in evaluation ? getNumber((evaluation as { altura?: unknown }).altura) : undefined,
      gorduraCorporal:
        'composicaoCorporal' in evaluation
          ? getNumber(
              (evaluation as { composicaoCorporal?: { percentualGordura?: unknown } })
                .composicaoCorporal?.percentualGordura
            )
          : undefined,
      imc: 'imc' in evaluation ? getNumber((evaluation as { imc?: unknown }).imc) : undefined,
      observacoes:
        'observacoes' in evaluation
          ? getText((evaluation as { observacoes?: unknown }).observacoes)
          : undefined,
    }));
  } catch (error) {
...
async function getPersonalStudentCapacityByCode(
  ...
  try {
    ...
    const queries = Array.from(codeVariants).map((value) =>
...
async function updateStudentStatus(
  ...
  try {
    ...
    if (isPersonalDeactivation) {
      await removeStudentFromPersonal(options.personalId as string, uid);
...
async function removeStudentFromPersonal(personalId: string, studentId: string): Promise<void> {
  try {
    const db = getFirebaseDb();
    ...
    console.error('Error removing student from personal:', error);
    throw error;
  }
}
...
async function getAdminOverview(): Promise<AdminOverview> {
  ...
  const [totalUsers, totalAdmins, totalPersonals, totalAcademies] = await Promise.all([
    getCount(usersRef),
    getCount(query(usersRef, where('admin', '==', true))),
    getCount(query(usersRef, where('professorAccount', '==', true))),
    getCount(query(usersRef, where('academyAccount', '==', true))),
  ]);

  const totalAlunos = Math.max(0, totalUsers - totalAdmins - totalPersonals - totalAcademies);
